...
from filament_layout import canonical_data_root
...
if __name__ == '__main__':
    ...
    parser.add_argument('--filament-id', type=str, default=os.environ.get('FILAMENT_ID', 'FIL001'), help="Filament identifier")
    parser.add_argument('--focals', nargs='*', type=int, default=[75, 120], help="Focal lengths in mm to analyze")
    parser.add_argument('--data-root', type=str, default=os.environ.get('DATA_ROOT', ''), help="Optional explicit dataset root")
    args = parser.parse_args()

    datasets = {}
    if args.data_root:
        focal_mm = int(args.focals[0]) if args.focals else 75
        datasets[f'{focal_mm}mm'] = {
            'path': os.path.join(os.path.abspath(args.data_root), 'raw')
            if os.path.isdir(os.path.join(os.path.abspath(args.data_root), 'raw'))
            else os.path.abspath(args.data_root),
            'focal_length': float(focal_mm) * 1000.0,
            'focal_mm': focal_mm,
        }
    else:
        for focal_mm in args.focals:
            data_root = canonical_data_root(args.filament_id, focal_mm)
            datasets[f'{focal_mm}mm'] = {
                'path': os.path.join(data_root, 'raw'),
                'focal_length': float(focal_mm) * 1000.0,
                'focal_mm': int(focal_mm),
            }
    ...
    for name, params in datasets.items():
        ...
        df['focal_mm'] = params['focal_mm']
        df['filament_id'] = args.filament_id
    ...
    if all_results:
        ...
        output_path = f'fft_analysis_results_{args.filament_id}.csv'
