...
from filament_layout import canonical_run_root
...
def _load_ablation_summary(seed_root_75: str, seed_root_120: str) -> pd.DataFrame:
    ...
    for focal, root in [(75, seed_root_75), (120, seed_root_120)]:
        candidates = [
            ...
            os.path.join(root, 'ablation_sweep'),
        ]
        sweep_root = next((cand for cand in candidates if os.path.isdir(cand)), '')
...
def main():
    ...
    if eval_filament_id != train_filament_id:
        raise ValueError('focal_length_comparison_outputs only supports same train/eval filament conditions; set EVAL_FILAMENT_ID equal to TRAIN_FILAMENT_ID.')

    seeds = [124, 212]

    # Baseline metrics for loss/score curves
    baselines = {
        (75, 124): os.path.join(canonical_run_root(train_filament_id, 75), 'baseline', 'seed_124', 'metrics.csv'),
        (120, 124): os.path.join(canonical_run_root(train_filament_id, 120), 'baseline', 'seed_124', 'metrics.csv'),
        (75, 212): os.path.join(canonical_run_root(train_filament_id, 75), 'baseline', 'seed_212', 'metrics.csv'),
        (120, 212): os.path.join(canonical_run_root(train_filament_id, 120), 'baseline', 'seed_212', 'metrics.csv'),
    }

    # Seed roots for ablation/sensitivity
    seed_roots = {
        (75, 124): os.path.join(canonical_run_root(train_filament_id, 75), 'ablation', 'seed_124'),
        (120, 124): os.path.join(canonical_run_root(train_filament_id, 120), 'ablation', 'seed_124'),
        (75, 212): os.path.join(canonical_run_root(train_filament_id, 75), 'ablation', 'seed_212'),
        (120, 212): os.path.join(canonical_run_root(train_filament_id, 120), 'ablation', 'seed_212'),
    }
    ...
    out = os.path.join(out_dir, f'Fig_results_network_training_ablation_train_{train_filament_id}_eval_{eval_filament_id}')
    fig.savefig(out + '.pdf')
    fig.savefig(out + '.png')
    ...
    df_75_124.assign(train_filament_id=train_filament_id, eval_filament_id=eval_filament_id, focal_mm=75, seed=124).to_csv(
        os.path.join(tab_dir, f'source_data_seed124_75mm_baseline_metrics_train_{train_filament_id}_eval_{eval_filament_id}.csv'),
        index=False,
    )
    df_120_124.assign(train_filament_id=train_filament_id, eval_filament_id=eval_filament_id, focal_mm=120, seed=124).to_csv(
        os.path.join(tab_dir, f'source_data_seed124_120mm_baseline_metrics_train_{train_filament_id}_eval_{eval_filament_id}.csv'),
        index=False,
    )
    df_75_212.assign(train_filament_id=train_filament_id, eval_filament_id=eval_filament_id, focal_mm=75, seed=212).to_csv(
        os.path.join(tab_dir, f'source_data_seed212_75mm_baseline_metrics_train_{train_filament_id}_eval_{eval_filament_id}.csv'),
        index=False,
    )
    df_120_212.assign(train_filament_id=train_filament_id, eval_filament_id=eval_filament_id, focal_mm=120, seed=212).to_csv(
        os.path.join(tab_dir, f'source_data_seed212_120mm_baseline_metrics_train_{train_filament_id}_eval_{eval_filament_id}.csv'),
        index=False,
    )
    df_ab.assign(train_filament_id=train_filament_id, eval_filament_id=eval_filament_id).to_csv(
        os.path.join(tab_dir, f'source_data_seed124_ablation_summary_train_{train_filament_id}_eval_{eval_filament_id}.csv'),
        index=False,
    )
    ...
    print(' ', out + '.png')
    print('Source data:')
    print(' ', os.path.join(tab_dir, f'source_data_seed124_75mm_baseline_metrics_train_{train_filament_id}_eval_{eval_filament_id}.csv'))
    print(' ', os.path.join(tab_dir, f'source_data_seed124_120mm_baseline_metrics_train_{train_filament_id}_eval_{eval_filament_id}.csv'))
    print(' ', os.path.join(tab_dir, f'source_data_seed212_75mm_baseline_metrics_train_{train_filament_id}_eval_{eval_filament_id}.csv'))
    print(' ', os.path.join(tab_dir, f'source_data_seed212_120mm_baseline_metrics_train_{train_filament_id}_eval_{eval_filament_id}.csv'))
    print(' ', os.path.join(tab_dir, f'source_data_seed124_ablation_summary_train_{train_filament_id}_eval_{eval_filament_id}.csv'))
