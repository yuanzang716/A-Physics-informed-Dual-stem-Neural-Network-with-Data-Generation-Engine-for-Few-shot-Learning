...
from filament_layout import (
    ...
    resolve_dataset_subpaths,
    resolve_filament_context,
)

LAYOUT = resolve_filament_context(
    default_data_root="/root/autodl-tmp/Diameter_100.2_120mm",
    default_gt=100.2,
    default_focal_mm=120,
    default_filament_id="FIL001",
)
# Data engine configuration (train/val physically isolated)
...
class DataConfig:
    ...
    # Mode
    # --------------------------
    # Fast verification mode: quickly check end-to-end pipeline
    FAST_VERIFY = False
    ...
    #     simulation_dataset/
    #
    # Set AUTO_SPLIT_FROM_SINGLE_FOLDER=True to auto-split from a single folder.
    FILAMENT_ID = LAYOUT.filament_id
    EVAL_FILAMENT_ID = LAYOUT.eval_filament_id
    FOCAL_MM = LAYOUT.focal_mm
    GT_DIAMETER_UM = LAYOUT.gt_diameter_um

    DATA_ROOT = LAYOUT.data_root
    RAW_FOLDER = LAYOUT.raw_dir
    TRAIN_REAL_FOLDER = LAYOUT.train_real_dir
    VAL_REAL_FOLDER   = LAYOUT.val_real_dir
    ...
    INIT_DIAMETER_FILE = os.path.join(ARTIFACTS_DIR, 'initial_diameter.txt')
    RESULTS_FILE = os.path.join(ARTIFACTS_DIR, 'optimized_params.csv')
    # Coarse real-parameter tables for conditioning
    TRAIN_REAL_RP_FILE = os.path.join(ARTIFACTS_DIR, 'train_real_rp.csv')
    VAL_REAL_RP_FILE   = os.path.join(ARTIFACTS_DIR, 'val_real_rp.csv')

    # Optimization settings for VAL real parameter estimation
    VAL_RP_OPT_MAXITER = 25
    VAL_RP_OPT_POPSIZE = 25

    # Dataset output directories
    DATASET_ROOT = LAYOUT.simulation_dataset_dir
    ...
    # Pre-generate validation diffs to disk
    ...
    FOCAL_LENGTH_UM = float(FOCAL_MM) * 1000.0
    PIXEL_SIZE_UM = 4.8
    ...
    _R_BOUNDS = LAYOUT.r_bounds_mm if LAYOUT.r_bounds_mm else (-15*1.05, -15*0.95)
    BOUNDS_BASE = [
        (-5.0, 0.0),        # phi (deg) allow small tilt (estimated on raw REAL)
        _R_BOUNDS,           # r (mm) — per-filament from filament_layout
        (900.0, 1600.0),    # b (um)
    ...
    # --------------------------
    # Soft prior on diameter 'a' toward the coarse FFT estimate.
    A_PRIOR_RATIO = 0.05
    A_REG_WEIGHT  = 0.5
    ...
    # Epsilon for SciPy DE bounds when A_TO_COARSE is True
    A_EPS_REL = 1e-6
    ...
    PERTURB_RATE = 0.05
    # Per-parameter perturbation floor to avoid zero-variance near zero
    ...
    # Overwrite existing generated data
    OVERWRITE_DATASET = False

    # --- RP table protection ---
    OVERWRITE_REAL_RP = True
    SIGN_RP_TABLES = False  # CSV integrity signatures
...
class TrainConfig:
    ...
    WORK_DIR = LAYOUT.cache_dir
    BASE_MODEL_SAVE_DIR = os.environ.get(
        "BASE_MODEL_SAVE_DIR",
        canonical_stage_dir(RUN_ROOT, "baseline", int(os.environ.get("SEED", "42"))),
    )
    ...
    INIT_DIAMETER_TXT = os.path.join(ARTIFACTS_DIR, "initial_diameter.txt")
    TRAIN_REAL_RP_CSV = os.path.join(ARTIFACTS_DIR, 'train_real_rp.csv')
    VAL_REAL_RP_CSV   = os.path.join(ARTIFACTS_DIR, 'val_real_rp.csv')

    FFT_CACHE_DIR = LAYOUT.fft_cache_dir

    EVAL_DATA_ROOT = os.environ.get(
        "EVAL_DATA_ROOT",
        canonical_data_root(EVAL_FILAMENT_ID, FOCAL_MM) if EVAL_FILAMENT_ID != FILAMENT_ID else DATA_ROOT,
    )
    _EVAL_SUBPATHS = resolve_dataset_subpaths(EVAL_DATA_ROOT)
    EVAL_REAL_FOLDER = os.environ.get("EVAL_REAL_FOLDER", _EVAL_SUBPATHS["val_real_dir"])
    EVAL_DATASET_ROOT = os.environ.get("EVAL_DATASET_ROOT", _EVAL_SUBPATHS["simulation_dataset_dir"])
    EVAL_LABELS_CSV = os.environ.get(
        "EVAL_LABELS_CSV",
        os.path.join(EVAL_DATASET_ROOT, "val", "dataset_labels_val.csv"),
    )
    EVAL_REAL_RP_CSV = os.environ.get(
        "EVAL_REAL_RP_CSV",
        os.path.join(_EVAL_SUBPATHS["artifacts_dir"], "val_real_rp.csv"),
    )
    EVAL_INIT_DIAMETER_TXT = os.environ.get(
        "EVAL_INIT_DIAMETER_TXT",
        os.path.join(_EVAL_SUBPATHS["artifacts_dir"], "initial_diameter.txt"),
    )
    EVAL_FFT_CACHE_DIR = os.environ.get(
        "EVAL_FFT_CACHE_DIR",
        _EVAL_SUBPATHS["fft_cache_dir"] if EVAL_FILAMENT_ID != FILAMENT_ID else FFT_CACHE_DIR,
    )
    ...
    GROUND_TRUTH_DIAMETER = GT_DIAMETER_UM
    NUM_EPOCHS = int(os.environ.get("NUM_EPOCHS", "60"))  # Read from env var, default 60
    BATCH_SIZE = int(os.environ.get("BATCH_SIZE", "32"))
    LEARNING_RATE = 1e-4
    WEIGHT_DECAY = 1e-3
    NUM_WORKERS = 8
    SEED = int(os.environ.get("SEED", "42"))  # Read from env var, default 42
    ...
    MAX_BATCHES = int(os.environ.get("MAX_BATCHES", "-1"))  # Limit batch count (-1 means unlimited; used for sensitivity quick tests)

    # --- Sensitivity Perturbation Parameters ---
    SENSITIVITY_COARSE_PERTURB = float(os.environ.get("SENSITIVITY_COARSE_PERTURB", "0.0"))  # Coarse diameter perturbation (relative value)
    SENSITIVITY_IMAGE_NOISE = float(os.environ.get("SENSITIVITY_IMAGE_NOISE", "0.0"))  # Image noise standard deviation
    SENSITIVITY_PARAMS_ZERO = os.environ.get("SENSITIVITY_PARAMS_ZERO", "0").strip() in {"1", "true", "True"}  # Parameter zeroing test

    # --- Ablation: Data Quantity ---
    SIM_SAMPLE_COUNT = int(os.environ.get("SIM_SAMPLE_COUNT", "-1"))  # Number of sim samples per real image (-1 = all)
    ...
    PINN_FOCLENGTH_UM = float(FOCAL_MM) * 1000.0
    ...
    # Include diff channel in the real branch input
    USE_DIFF_INPUT = False
    # Whether to include FFT features as input (2nd channel for real / sim / diff)
    USE_FFT_INPUT = True
    ...
    # Stabilization switches (configurable via environment variables)
    ...
    # Reverse warmup: ramp selected loss weights from target to reduced over first K epochs
    ...
    # Adaptive pair loss boosting when pair loss exceeds threshold
    ...
    # Gradually ramp up auxiliary losses after pair-only period
    ...
    # Enable auxiliary losses only when pair loss stabilizes below threshold
    ...
    # Pair weight multiplier for model selection score
...
def preprocess_image_basic(img_path: str) -> np.ndarray:
    """Read image, convert to grayscale, flip, and crop borders."""
...
def validate_real_folders():
    ...
    if os.path.isdir(DataConfig.TRAIN_REAL_FOLDER):
        ...
        if n_bmp == 0:
            if DataConfig.AUTO_SPLIT_FROM_SINGLE_FOLDER:
                print("[CHECK] TRAIN_REAL_FOLDER is currently empty and will be populated by AUTO_SPLIT.")
            else:
...
def _optimize_single_image(I_real: np.ndarray, bounds: List[Tuple[float, float]], consts, a_prior: Optional[float],
    ...
    def _cb(xk, convergence):
        ...
        # Control print frequency: at least once per generation, throttled to every 2s to avoid flooding
        if (now - last_print_t[0]) >= 2.0 or gen[0] == 1:
            # Do not recompute the objective here (expensive); just show convergence + elapsed time
    ...
    res = differential_evolution(
        ...
        polish=True,
        init='latinhypercube',
        disp=True,             # SciPy built-in per-generation output (sometimes more concise/frequent)
        callback=_cb           # Progress callback
    )
...
def optimize_real_folder(
    ...
    for i, f in enumerate(bmp_files):
        ...
        try:
            # Use raw image; alignment and masking are applied before network input.
...
def run_optimization(init_a: float) -> pd.DataFrame:
    ...
    # Post-process optimized parameters
...
def run_generator(df_opt_train: pd.DataFrame):
    ...
    if DataConfig.FAST_VERIFY and bool(DataConfig.FAST_VERIFY_SKIP_VAL):
        ...
        if DataConfig.VAL_SELF_SIM and val_files:
            ...
            for f in val_files:
                fname = os.path.basename(f)
                # Load raw image; optionally align after reading rp table
...
def clean_runtime_artifacts(config):
    """Clean up temporary files generated at runtime (disabled by default)."""
    # Cleanup is disabled by default to preserve training results.
    # To enable, set the environment variable FORCE_CLEAN_ARTIFACTS=1
    force_clean = os.environ.get("FORCE_CLEAN_ARTIFACTS", "0").strip() == "1"
    if not force_clean:
        print(f"[Clean] Skipping cleanup (set FORCE_CLEAN_ARTIFACTS=1 to enable)")
        return
    ...
    for root, dirs, files in os.walk(config.BASE_MODEL_SAVE_DIR):
        ...
        for file in files:
            ...
            # Only delete temporary files; do not delete key result files
            if file.endswith(".csv") or file.endswith(".pth") or file.endswith(".png"):
                # Extra protection: do not delete key files like best_model.pth, best_ema_model.pth, metrics.csv
                ...
                try:
                    ...
                    pass
    print(f"[Clean] Done, removed {removed_count} temporary file(s)")
...
def get_loss_weight_scales(config, epoch: int, pair_only_epochs: int = 0, pair_loss_history: list = None, conditional_enable: bool = False) -> Dict[str, float]:
    ...
    # Reverse warmup: ramp from 1 -> min_scale (reduce phys/weak early, let pair dominate)
...
class AdvancedPhysicsLoss(nn.Module):
    ...
    def forward(self, pred_img, target_img):
        # Instance-normalize before computing physics sub-losses
...
class FFTPrecomputer:
    ...
    def run(self):
        ...
        # Scan and regenerate missing or corrupted tensors
...
class DiffSimRealDataset(Dataset):
    def __init__(self, dataset_root, real_root, labels_csv, init_diameter_txt,
        ...
        cache_base = os.path.abspath(fft_cache_dir or config.FFT_CACHE_DIR)
        self.cache_root = os.path.join(cache_base, f"size_{config.RESIZE}")
    ...
    def _get_sim_params(self, idx):
        """Return SIM params in physical units [phi, r, b, x, gamma]."""
    ...
    def __getitem__(self, idx):
        ...
        # full_input = [real(2), sim(2), diff(2)]
...
class FraunhoferPhysicsLayer(nn.Module):
    ...
    def forward(self, pred_diameter_um, norm_params, target_real_img=None):
        # FraunhoferPhysicsLayer expects physical units (no normalization inside).
...
class DualStemNet(nn.Module):
    ...
    def _init_convnext_tiny(self, emb_dim: int):
        ...
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        # Infer feature dimension via dummy forward pass
    ...
    def forward_real(self, x_real4: torch.Tensor, params10: torch.Tensor, coarse1: torch.Tensor, return_stem_pool: bool = False):
        ...
        # Residual log-space diameter prediction
    ...
    def forward_sim(self, x_sim2: torch.Tensor, params10: torch.Tensor, coarse1: torch.Tensor, return_stem_pool: bool = False):
        # Single-branch ablation: route the sim branch through stem_real (pad to 4ch)
        # Enabled via FORCE_SINGLE_BRANCH=1 environment variable
...
def weak_guidance_rel(pred, coarse, tolerance=0.1):
    # Continuous relative-normalized deviation outside tolerance band
...
def validate_unified(model, physics, val_loader, config, adv_phys_loss):
    ...
    # Simplified output: no validation progress bar (consistent with the training loop)
    ...
    with torch.no_grad():
        for fnames, full_input, sim_pt, rp, sp, coarse, target_a, is_paired, real_idx in val_loader:
            ...
            if config.ENABLE_PHYS:
                # Physics loss on normalized intensities
    ...
    # Group variance (reported, not used in selection)
    group_vars = []
    ...
    # Apply pair weight boost for model selection
...
def save_debug_visualization(model, physics, loader, config, epoch, out_dir):
    ...
    try:
        ...
        with torch.no_grad():
            ...
            pred_img = img_pred[0, :, :, :]
            # Flip sim image to match real image display orientation
...
# =========================================================
# 13.2 Dataset PNG integrity and incremental repair
# =========================================================
...
def ensure_main_required_artifacts(config: 'TrainConfig', *, attempt: int = 1) -> None:
    """Regenerate missing or corrupt data-pipeline artifacts and rebuild FFT cache."""
...
def main(config: TrainConfig):
    ...
    # Ensure the output directory exists (passed via environment variable by run_all_experiments.py)
    ...
    # PCGrad/GradNorm use manual grad handling in fp32; forward-only autocast is optional.
    ...
    print("Strategy config | " + " | ".join(strategy_info))

    # Sensitivity mode: load model weights from baseline
    if config.SENSITIVITY_MODE:
        if not config.BASELINE_DIR or not os.path.exists(config.BASELINE_DIR):
            raise ValueError(f"Sensitivity mode requires BASELINE_DIR, but directory does not exist: {config.BASELINE_DIR}")
        ...
        if os.path.exists(baseline_model_path):
            ...
            print(f"[Sensitivity] Model loaded successfully")
            ...
            raise FileNotFoundError(f"Baseline model file not found: {baseline_model_path}")
    ...
    for epoch in range(1, config.NUM_EPOCHS + 1):
        ...
        # Simplified output: no tqdm progress bar
        epoch_start_time = time.time()
        if epoch == 1:
            if config.SENSITIVITY_MODE:
                print(f"[Sensitivity] Starting evaluation, {config.NUM_EPOCHS} epoch(s) total")
                if config.MAX_BATCHES > 0:
                    print(f"[Sensitivity] Limiting each epoch to {config.MAX_BATCHES} steps (batches), batch_size={config.BATCH_SIZE}")
            else:
                print(f"Starting training, {config.NUM_EPOCHS} epoch(s) total...")
        
        batch_count = 0
        for fnames, full_input, sim_pt, rp, sp, coarse, target_a, is_paired, real_idx in train_loader:
            # Sensitivity mode: limit batch count
            ...
            with torch.amp.autocast('cuda', enabled=autocast_enabled):
                ...
                # Early pair-only mode: disable auxiliary losses for the first N epochs
                ...
                # TTA and group embedding consistency losses
                ...
                # Calculate effective loss weights
            ...
            # Skip backward pass in sensitivity mode or when loss has no gradient
...
def export_predictions(config, ckpt_path: str, split: str, out_csv: str, use_ema_state: bool = False):
    ...
    if split == 'train':
        ...
        dataset_root = config.DATASET_ROOT
        init_diameter_txt = config.INIT_DIAMETER_TXT
        fft_cache_dir = config.FFT_CACHE_DIR
        source_filament_id = config.FILAMENT_ID
        source_split_role = 'train_real'
    elif split == 'val':
        labels_csv = config.VAL_LABELS_CSV
        real_root = config.VAL_REAL_FOLDER
        real_rp_csv = config.VAL_REAL_RP_CSV
        ...
        fft_cache_dir = config.FFT_CACHE_DIR
        source_filament_id = config.FILAMENT_ID
        source_split_role = 'internal_val'
    elif split == 'eval':
        labels_csv = config.EVAL_LABELS_CSV
        real_root = config.EVAL_REAL_FOLDER
        real_rp_csv = config.EVAL_REAL_RP_CSV
        dataset_root = config.EVAL_DATASET_ROOT
        init_diameter_txt = config.EVAL_INIT_DIAMETER_TXT
        fft_cache_dir = config.EVAL_FFT_CACHE_DIR
        source_filament_id = config.EVAL_FILAMENT_ID
        source_split_role = 'external_eval'
    else:
    ...
    ds = DiffSimRealDataset(
        ...
        real_root=real_root,
        labels_csv=labels_csv,
        init_diameter_txt=init_diameter_txt,
    ...
    with torch.no_grad():
        for fnames, full_input, sim_pt, rp, sp, coarse, target_a, is_paired, real_idx in tqdm(loader, desc=f"Predict {split}", ncols=80):
            ...
            for f, d in zip(fnames, d_final):
                rows.append({
                    "split": split,
                    "source_split_role": source_split_role,
                    "source_filament_id": source_filament_id,
                    "requested_eval_filament_id": config.EVAL_FILAMENT_ID,
                    "real_root": os.path.abspath(real_root),
                    "dataset_root": os.path.abspath(dataset_root),
                    "labels_csv": os.path.abspath(labels_csv),
                    "fname": str(f),
                    "real_id": int(_extract_real_id(str(f))),
                    "pred_um": float(d),
                })

    pd.DataFrame(rows).to_csv(out_csv, index=False)

def ensure_eval_fft_cache(config: TrainConfig):
    if config.EVAL_FILAMENT_ID == config.FILAMENT_ID:
        return

    class _EvalCacheConfig:
        pass

    eval_cfg = _EvalCacheConfig()
    eval_cfg.CROP_PIXELS = config.CROP_PIXELS
    eval_cfg.RESIZE = config.RESIZE
    eval_cfg.APPLY_REAL_ALIGN = config.APPLY_REAL_ALIGN
    eval_cfg.APPLY_CENTER_MASK = config.APPLY_CENTER_MASK
    eval_cfg.CENTER_MASK_BAND_PX = config.CENTER_MASK_BAND_PX
    eval_cfg.CENTER_MASK_MODE = config.CENTER_MASK_MODE
    eval_cfg.FFT_CACHE_DIR = config.EVAL_FFT_CACHE_DIR
    eval_cfg.FFT_REAL_SUBDIR = config.FFT_REAL_SUBDIR
    eval_cfg.FFT_SIMDIFF_SUBDIR = config.FFT_SIMDIFF_SUBDIR
    eval_cfg.FFT_CACHE_PNG_PREFIXES = config.FFT_CACHE_PNG_PREFIXES
    eval_cfg.TRAIN_REAL_FOLDER = config.EVAL_REAL_FOLDER
    eval_cfg.VAL_REAL_FOLDER = config.EVAL_REAL_FOLDER
    eval_cfg.DATASET_ROOT = config.EVAL_DATASET_ROOT
    eval_cfg.TRAIN_REAL_RP_FILE = config.EVAL_REAL_RP_CSV
    eval_cfg.VAL_REAL_RP_FILE = config.EVAL_REAL_RP_CSV

    FFTPrecomputer(eval_cfg).run()
...
def run_export_predictions():
    ...
    split_role = os.environ.get(
        'SPLIT_ROLE',
        'external_eval' if cfg.EVAL_FILAMENT_ID != cfg.FILAMENT_ID else 'internal_val',
    )
    if split_role == 'external_eval':
        ensure_eval_fft_cache(cfg)
    out_root = os.environ.get('EXPORT_OUT_DIR', default_export_out_dir(base_dir, cfg.EVAL_FILAMENT_ID, split_role))
    os.makedirs(out_root, exist_ok=True)

    seed = os.environ.get('SEED', 'unknown')
    include_internal_with_external = os.environ.get(
        'EXPORT_INCLUDE_INTERNAL_SPLITS_FOR_EXTERNAL',
        '0',
    ).strip().lower() in ('1', 'true', 'yes', 'y')

    if split_role == 'external_eval' and not include_internal_with_external:
        for stale_path in glob.glob(os.path.join(out_root, 'pred_seed*_*.csv')):
            if stale_path.endswith('_eval.csv'):
                continue
            try:
                os.remove(stale_path)
            except OSError:
                pass
        export_predictions(cfg, ckpt_path, 'eval', os.path.join(out_root, f'pred_seed{seed}_{ckpt_tag}_eval.csv'), use_ema_state=use_ema)
    else:
...
if __name__ == "__main__":
    ...
    # Require explicit opt-in for training
    allow_train = os.environ.get('ALLOW_TRAIN', '0').strip().lower() in ('1', 'true', 'yes', 'y')
    if not allow_train:
        raise RuntimeError(
            "Training is disabled by default to prevent unintended GPU cost. "
    ...
    try:
        ...
        snapshot_env_keys = [
            ...
            'FILAMENT_ID', 'EVAL_FILAMENT_ID', 'FOCAL_MM', 'GT_DIAMETER_UM', 'DATA_ROOT', 'RUN_ROOT', 'REPORT_ROOT', 'EVAL_FFT_CACHE_DIR',
        ...
        snapshot.update(
            build_snapshot_payload(
                LAYOUT,
                split_role=os.environ.get('SPLIT_ROLE', 'internal_val'),
                extra={
                    'eval_data_root': getattr(TrainConfig, 'EVAL_DATA_ROOT', None),
                    'eval_fft_cache_dir': getattr(TrainConfig, 'EVAL_FFT_CACHE_DIR', None),
                    'gt_diameter_um': getattr(TrainConfig, 'GROUND_TRUTH_DIAMETER', None),
                },
            )
        )

        # Record hash of previous best_model for integrity check.
