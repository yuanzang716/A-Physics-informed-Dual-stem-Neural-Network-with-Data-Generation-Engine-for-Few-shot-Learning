...
class DataConfig:
    ...
    _R_BOUNDS = LAYOUT.r_bounds_mm if LAYOUT.r_bounds_mm else (-5*1.05, -5*0.95)
    BOUNDS_BASE = [
...
def preprocess_image_basic(img_path: str) -> np.ndarray:
    """Read image, to gray, flip, crop borders."""
...
def _optimize_single_image(I_real: np.ndarray, bounds: List[Tuple[float, float]], consts, a_prior: Optional[float],
    ...
    def _cb(xk, convergence):
        ...
        # Print callback for DE progress
...
def run_generator(df_opt_train: pd.DataFrame):
    ...
    print("="*60)
    # Skip generation if dataset is already complete
...
def clean_runtime_artifacts(config):
    ...
    # Sensitivity mode: ensure baseline/ablation/probe directories are not deleted
    if config.SENSITIVITY_MODE:
        ...
        for pattern in protected_patterns:
            if pattern in base_save_abs:
                print(f"[Clean] Warning: BASE_MODEL_SAVE_DIR contains a protected directory: {base_save_abs}")
...
class DualStemNet(nn.Module):
    ...
    def _init_convnext_tiny(self, emb_dim: int):
        ...
        # Expose pseudo layer1..4 for progressive freeze
